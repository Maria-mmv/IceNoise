...
logger = logging.getLogger('channelIceNoiseAdder')
...
class channelIceNoiseAdder:
    ...
    def get_temperature_ice(self,depth,model = "SP1"):
        ...
        if (model == "GL1" or  model == "GL2"):
            folder = '/home/masha/Documents/code/third_year/Antarctica_detectors/My_nuradiomc/ice_noise/'
    ...
    def run(
        ...
        for channel in station.iter_channels():
            ...
            noise_spec_sum = np.zeros((3, freq.shape[0]), dtype=complex)
            channel_spectrum = np.zeros(len(freq), dtype=complex)
            antenna_orientation = detector.get_antenna_orientation(station_id, cid)
            fmax = sampling_rate/2
            # Add ice noise
            for theta in thetas:
                flux_sum, zeniths ,sol_param = self.IceNoise_vs_Angle(theta, self.__d_theta,z_antenna,freq,self.__R ,self.__n_r,self.__r0 ,ice,model_ice,fmax)
                ...
                for azimuth in phis:
                    ...
                    noise_spectrum = np.zeros((3, freq.shape[0]), dtype=complex)
                    ...
                    antenna_response = antenna_pattern.get_antenna_response_vectorized(freq, theta, azimuth, *antenna_orientation)
